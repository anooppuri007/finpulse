...
if feature == "Dashboard":
    ...
    st.markdown("## Institutional Flow & Enterprise Risk Engine")
        ...
        if not success:
            ...
            if isinstance(data, list) and len(data) >= 2:
                try:
                    ...
                    # Styled Flow Grid
                    st.markdown(f"""
                    <div style="display:flex; justify-content:space-between; margin-bottom: 20px; background-color: var(--card-bg); padding: 15px; border-radius: 8px; border: 1px solid var(--border-color);">
                        <div style="text-align:center; width:33%;">
                            <div style="font-size: 0.9rem; color: var(--text-muted);">Foreign Capital (FII)</div>
                            <div style="font-size: 1.5rem; font-family: 'Courier New', monospace; font-weight: bold; color: {'#16A34A' if fii_val > 0 else '#DC2626'};">₹{fii_val:,.2f} Cr</div>
                        </div>
                        <div style="text-align:center; width:33%; border-left: 1px solid var(--border-color); padding-left: 10px;">
                            <div style="font-size: 0.9rem; color: var(--text-muted);">Domestic Allocation (DII)</div>
                            <div style="font-size: 1.5rem; font-family: 'Courier New', monospace; font-weight: bold; color: {'#16A34A' if dii_val > 0 else '#DC2626'};">₹{dii_val:,.2f} Cr</div>
                        </div>
                        <div style="text-align:center; width:33%; border-left: 1px solid var(--border-color); padding-left: 10px;">
                            <div style="font-size: 0.9rem; color: var(--text-muted);">Net Flow</div>
                            <div style="font-size: 1.5rem; font-family: 'Courier New', monospace; font-weight: bold; color: {'#16A34A' if net_val > 0 else '#DC2626'};">₹{net_val:,.2f} Cr</div>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    fig_waterfall.update_layout(height=250, margin=dict(l=0, r=0, t=30, b=0), title="Today's Liquidity Breakdown (Cr)")
                    ...
                    cum_net = np.cumsum(sim_net)
                    z_score = (net_val - np.mean(sim_net)) / (np.std(sim_net) + 1e-5)
                    ...
                    fig_area.add_trace(go.Scatter(
                        x=dates, y=cum_net, fill='tozeroy', 
                        mode='lines', line=dict(color='#8B5CF6', width=2),
                        name='Cumulative Net Flow',
                        fillcolor='rgba(139, 92, 246, 0.2)'
                    ))
                    fig_area.add_trace(go.Bar(
                        x=dates, y=sim_net, name='Daily Net', 
                        marker_color=['#16A34A' if val > 0 else '#DC2626' for val in sim_net],
                        opacity=0.5
                    ))
                    fig_area.update_layout(title="Simulated 30-Day Cumulative Net Flow (Cr)", height=300, margin=dict(l=0, r=0, t=40, b=0), showlegend=False)
    ...
    with col_events:
        ...
        st.markdown("### 📊 Economy Pulse (Official)")
        st.caption("As of 12-Jun-2026 · Source: RBI/MOSPI/AMFI")
        pulse_cols = st.columns(2)
        pulse_cols[0].metric("Repo Rate", "6.50%", "Neutral", delta_color="off")
        pulse_cols[1].metric("CPI Inflation", "3.93%", "Target: 4%", delta_color="off")
        pulse_cols[0].metric("GDP Growth (FY24)", "8.2%", "Robust", delta_color="off")
        pulse_cols[1].metric("MF SIP Inflows", "₹30,954 Cr", "+ Record High", delta_color="normal")
        
        st.markdown("### 📡 Live Stress Indicators")
        st.caption("Live · Yahoo Finance")
        vix = fetch_yahoo_price("^INDIAVIX")
        st.markdown(f"**India VIX (Volatility Index):** {vix['price']:.2f} ({vix['pct_change']:+.2f}%)")
    ...
    with col_risk:
        ...
        fig_risk.update_layout(margin=dict(l=0, r=0, t=10, b=0), height=200, xaxis_title="Estimated Nifty Impact %", yaxis_title="")
